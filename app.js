(function () {
  ...
  function aromaterapiaBlend(ctx) {
    ...
    if (/cefale|enxaquec|cabe[çc]a/.test(ctx)) return { blend: "Hortelã-pimenta 1gt + Lavanda 2gts + Manjerona 1gt (em 10 mL OV)", posologia: "Aplicar pouca quantidade na nuca/templos 1–2×/dia; evitar olhos.", cuidados: "Evitar hortelã em <6 anos, gestantes e epilépticos." };
    ...
    if (/tpm|c[oó]licas? menstruais|femin|menop/.test(ctx)) return { blend: "Gerânio 2gts + Lavanda 2gts + Sálvia-esclareia 1gt", posologia: "Difusor 20–30 min/dia; tópico abdome baixo: 3 gts em 10 mL OV, 1–2×/dia.", cuidados: "Sálvia-esclareia: evitar na gestação." };
    if (/rinite|sinus|congest/.test(ctx)) return { blend: "Eucalipto globulus 1gt + Tea tree 1gt + Lavanda 2gts + Limão 1gt", posologia: "Difusor 15–20 min, 2×/dia. Inalação a vapor (1–2 gts).", cuidados: "Limão fotossensível; evitar eucalipto em <6 anos." };
    if (/foco|estudo|aten[cç][aã]o|concentra/.test(ctx)) return { blend: "Alecrim qt. cineol 1gt + Hortelã-pimenta 1gt + Laranja-doce 2gts", posologia: "Difusor 20 min durante estudo; inalação breve antes de tarefas.", cuidados: "Evitar em gestantes/hipertensos/epilépticos." };
    if (/tristeza|des[aâ]nimo|apatia/.test(ctx)) return { blend: "Laranja-doce 2gts + Bergamota 2gts + Olíbano 1gt", posologia: "Difusor manhã/tarde 15–20 min; inalação breve quando precisar.", cuidados: "Ventilar o ambiente; cítricos na pele + sol, não." };
    return { blend: "Lavanda 3gts + Laranja-doce 2gts", posologia: "Difusor 20–30 min 1–2×/dia.", cuidados: "Cítricos + sol não." };
  }
